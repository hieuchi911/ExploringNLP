...
class MegaMerge(layers.Layer):
    ...
    def call(self, h, c2q, q2c, max_context_length):
        ...
        def mega_merge(inputs):
            h, c2q, q2c = inputs
            for c in range(0, max_context_length):
                # beta below is a row vector of size (1, 800)
                beta = tf.keras.layers.concatenate([tf.expand_dims(h[c], 1), tf.expand_dims(tf.transpose(c2q)[c], 1), tf.math.multiply(tf.expand_dims(h[c], 1), tf.expand_dims(tf.transpose(c2q)[c], 1)), tf.math.multiply(tf.expand_dims(h[c], 1), tf.expand_dims(tf.transpose(q2c)[c], 1))], 0) # 0 here indicates that concatenation is done along rows, since each element in the concatenate func is of size (200,)
                if c == 0:
                    ...
                    G = tf.keras.layers.concatenate([G, beta], 1)
            return G
        G = tf.keras.backend.map_fn(mega_merge, (h, c2q, q2c), dtype = 'float32')
        print("G is: ", G)
